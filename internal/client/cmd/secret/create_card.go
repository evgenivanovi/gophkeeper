...
type CreateCardViewSettings struct {
...
func ProvideCreateCardViewSettings() CreateCardViewSettings {
...
type CreateCardModel struct {
	...
	result  *CreateCardResultMsg
}

func ProvideCreateCardModel(
	op common.Options,
	args CreateSecretArg,
	usecase secret.CreateDecodedSecretUsecase,
) *CreateCardModel {
	...
	md := &CreateCardModel{
		...
		view:   ProvideCreateCardViewSettings(),
...
func (m *CreateCardModel) updateInputs(msg tea.Msg) tea.Cmd {
	commands := make([]tea.Cmd, len(m.inputs))
...
func (m *CreateCardModel) Execute() tea.Cmd {
	ctx := context.Background()
	ctx = common.OptionsWithCtx(ctx, m.op)

	req := m.buildRequest(m.args)
	return ExecuteCreateCard(m.usecase, ctx, req)
}

func (m *CreateCardModel) buildRequest(
	args CreateSecretArg,
) secretsharedmd.DecodedSecretDataModel {

	num := m.inputs[0].Value()
	cvv := m.inputs[1].Value()
	due := m.inputs[2].Value()

	return secretsharedmd.DecodedSecretDataModel{
		Name: args.Name,
		Type: "CARD",
		Content: &secretsharedmd.CardSecretContentModel{
			Num: num,
			CVV: cvv,
			Due: due,
		},
...
func (m *CreateCardModel) resume() string {
...
func (m *CreateCardModel) finish() string {
	...
	if !m.result.IsError() {
		output := strings.Builder{}
		output.WriteString("Card created!")
		output.WriteString(std.NL)
		return output.String()
	}
...
func (msg *CreateCardResultMsg) IsError() bool {
	return msg.err != nil
}

func ExecuteCreateCard(
