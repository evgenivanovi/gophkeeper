...
func CreateAccessToken(data token.TokenData, expiration time.Duration) *jwt.Token {
	return auth.CreateAccessToken(
		MapUserFrom(data), expiration,
	)
}

func CreateRefreshToken(data token.TokenData, expiration time.Duration) *jwt.Token {
...
func MapUserFrom(data token.TokenData) auth.User {
...
func MapUserTo(entity auth.User) token.TokenData {
